import React, { useEffect, useState } from 'react';
import { useDispatch } from 'react-redux';
import { done, efface } from '../Actions/taskActions';
import { InputGroup, FormControl, Button } from 'react-bootstrap'
import 'bootstrap/dist/css/bootstrap.min.css';
import Edittask from './Edittask';

const Task = ({ el, choix }) => {
    const dispatch = useDispatch()
    const [checked, setChecked] = useState(false);
    const checkdone = (e) => {
        dispatch(done(e.target.id));
        setChecked(!checked);
        el.isDone=!el.isDone
        choix = !choix

    }

    const handelclick = (e) => {
        dispatch(efface(e.target.id))
        console.log(e.target.id)
    }

    return (
        <div>

            <div>
                <InputGroup className="mb-3">
                    
                    {el.isDone ? <p style={{ textDecoration: "line-through" }} >
                        
                    {el.description}</p> : <div><Edittask idel={el.id} desc={el.description} /></div>}
                    
                    <InputGroup.Append>
                        <InputGroup.Checkbox aria-label="Checkbox for following text input" defaultChecked={checked}
                            id={el.id} onChange={checkdone} value={el.isDone} />

                        <Button variant="outline-secondary" id={el.id} onClick={handelclick}><i class="bi bi-trash"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
                            <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z" />
                            <path fill-rule="evenodd" d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z" />
                        </svg></i>Delete</Button>
                    </InputGroup.Append>
                </InputGroup>
            </div>

        </div>
    );
}

export default Task
